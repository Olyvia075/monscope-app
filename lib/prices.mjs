...
// Portfolio, vault and LP pricing all ask for prices independently now. Firing
// those at GeckoTerminal concurrently gets us 429s, and a rate-limited answer is
// indistinguishable from "no price" — that silently dropped a $9.5k token from
// net worth. One at a time: the per-token cache means the second caller mostly
// hits cache rather than waiting on a real request.
let tailQueue = Promise.resolve()
export function priceTail(addresses) {
  const run = tailQueue.then(() => priceTailInner(addresses), () => priceTailInner(addresses))
  tailQueue = run.catch(() => {})
  return run
}

async function priceTailInner(addresses) {
  const now = Date.now()
  const prices = {}
  const misses = []
