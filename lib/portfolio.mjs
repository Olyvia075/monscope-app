...
export async function getPortfolio(input) {
  ...
  const [native, cg, discovered] = await Promise.all([
    client.getBalance({ address }), priceAnchors(), discoverTokens(address),
  ])

  // Curated registry first (it carries the CoinGecko anchors), then anything the
  // wallet has actually touched that the registry does not know about.
