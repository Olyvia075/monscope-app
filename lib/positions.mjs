...
// same anchor mapping the portfolio uses, so both value a token identically
const ANCHOR_CG = {
...
// Price a vault's underlying using the same anchors/DEX index as the rest of the
// app, so a vault position is valued on exactly the same basis as a plain holding.
// vaults over native MON report this sentinel rather than a real token address
const NATIVE_SENTINEL = '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee'

async function priceUnderlying(addresses) {
  const out = {}
  if (!addresses.length) return out
  try {
    const lookup = addresses.filter(a => a !== NATIVE_SENTINEL)
    ...
    for (const a of addresses) {
      ...
      const anchor = reg && ANCHOR_CG[reg.coingecko]
      if (anchor && cg[anchor]?.usd != null) out[a] = cg[anchor].usd
      else if (tail[a]) out[a] = tail[a].usd
    }
  } catch { /* unpriced underlyings simply stay absent */ }
  return out
}
...
export async function getPositions(address) {
  ...
  if (held.length) {
    ...
    if (vaults.length) {
      ...
      const priced = await priceUnderlying(vaults.map(v => v.asset.toLowerCase()))
