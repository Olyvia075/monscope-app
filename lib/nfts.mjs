...
// Promise-cached like the other shared reads: the positions adapter also needs
// the wallet's NFTs (to spot Uniswap-style LP managers) and must not pay for a
// second full history fetch.
const cache = new Map() // address(lower) -> Promise<nft[]>

export function getNfts(address) {
  const key = getAddress(address).toLowerCase()
  if (cache.has(key)) return cache.get(key)
  const p = readNfts(address)
  cache.set(key, p)
  setTimeout(() => cache.delete(key), 30_000).unref?.()
  return p
}

async function readNfts(address) {
...
async function fromHistory(client, a, al) {
  ...
  for (let page = 1; page <= 3; page++) {
    ...
    } catch { break } // keep the pages we did get
    rows.push(...batch)
